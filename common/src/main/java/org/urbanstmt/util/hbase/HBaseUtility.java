...
public class HBaseUtility {
	...
	public static final byte[] TWEET_CREATEDAT_COL = Bytes
			.toBytes("CREATED_AT");
	...
	private static List<Field> getAllFields(List<Field> fields, Class<?> c,
		...
		for (Field field : c.getDeclaredFields()) {
			...
			System.out.println("field type for nested=" + field.getType()
					+ ", condition="
					+ (field.getType().equals(CoordinatesVO.class)) + " or ="
					+ (field.getType().equals(PlaceVO.class)));
			if (UtilFunctions.isCustomClass(field.getType())) {
				...
				try {
					...
					System.out.println("My nested expected type ="
							+ field.getType() + ", " + field.getName() + ", "
							+ m.getName() + ", " + topObj.getId() + ", "
							+ nestedObj);
					if (nestedObj != null) {
	...
	public static byte[] buildPartialAnalyticsTCRowKey(String stime)
			throws AnalyticsStoreException {
		byte[] rowKey;
		try {

			byte[] time = Bytes.toBytes(stime);
			byte[] type = Bytes
					.toBytes(ConstantsAndEnums.AnalysisType.TERMS_COUNT
							.toString());

			int keyLength = time.length + type.length;
			rowKey = new byte[keyLength];
			ByteBuffer target = ByteBuffer.wrap(rowKey);
			target.put(time);
			target.put(type);
			...
			throw new AnalyticsStoreException("Error creating id row key=" + ex);
		}
		return rowKey;
	}
